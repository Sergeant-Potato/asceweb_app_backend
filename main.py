...
'''Blacklisted IPs and Hostnames!!!! XD'''
__bp = [
    "127.0.0.1",
    "localhost"
]
...
def loginAdmin(userName:str, passwd: SecretStr, r: Request, db: Session = Depends(get_db)):
...
def createAdmin(userName:str, passwd:str, name:str, email:str, phone: str, adminLevel: str, token: str, r: Request, db: Session = Depends(get_db)):
...
def competitionSignUp(name: str, email: str, asce_member:str ,ascemembership_number: str, competition_name: str, courses:str, experiences: str,daily_availability: str, travel_availability: str, travel_june:str,older_than_twentyfive:str,heavy_driver:str, official_driver:str, r: Request, db: Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def chapterSignUp(name: str, email: str, phone:str, tshirt_size: str, age: int, bachelor:str, department: str, Academic_Years: int, r: Request, db: Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def getAdmins(masterAdminToken: str, r: Request, db: Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
    ...
    except (HTTPException,DecodeError,InvalidSignatureError, Exception) as e:
...
def getMembers(masterAdminToken: str, r: Request, db: Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def getCompetitionsMembers(masterAdminToken: str, r:Request, db: Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def isTokenValid(masterAdminToken:str, r: Request, db:Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def updateAdmins(r: Request, userName: str, masterAdminToken: str, newPasswd: str = None, newEmail: str = None,newPhone: str = None, newLevel: str = None, db: Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def updateMembers(r: Request, token: str,email: str, newEmail: str = None, newPhone:str = None, newTshirt_size: str = None, newAge: int = None, newBachelor:str = None, newDepartment: str = None, newAcademic_Years: int = None, newMembership: str = None, db: Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def deleteAdmin(masterAdminToken: str, email: str, r: Request, db:Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def deleteMembers(masterAdminToken: str, email: str, r:Request,  db:Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
...
def deleteCompetitions(masterAdminToken: str, email: str, r:Request, db:Session = Depends(get_db)):
    try:
        if r.client.host in __bp:
            raise Exception
